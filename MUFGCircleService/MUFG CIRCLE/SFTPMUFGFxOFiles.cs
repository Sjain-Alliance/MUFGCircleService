...
namespace MUFGCircleService
{
    class SFTPMUFGFxOFiles
    {
        ...
        public SFTPMUFGFxOFiles(ILogger<Worker> _logger)
        {
            this._logger = _logger;

            try
            {
                DataSet ds = DBRepository.FillUpDataSetFromSp(EODModel.ConnectionString, "[ExportData].[getMUFGCircleAdaptorForFxOTrades]", null);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    this._logger.LogInformation("DataSet is not Blank for MUFG Trade File for FXOPTION");
                    GetSFTPDetails();
                    ExportMUFGCircleFiles(ds);
                }
                else
                {
                    this._logger.LogInformation("DataSet is Blank for MUFG Trade Files (FXOPTION) and we don't have to send them the file for now. So suppressing the File Generation");
                }
            }
            catch (Exception ex)
            {
        ...
        public void GetSFTPDetails()
        {
            ...
            _logger.LogInformation("Get SFTP Credenitals Successfully for MUFGCircle for fx option");
            if (dtSFTPCred != null)
            {
        ...
        public void ExportMUFGCircleFiles(DataSet ds)
        {
            ...
            string mufgCircleFileName = "RVUAT_MUFG_Circle_FXO_" + DateTime.Now.ToString("yyyyMMdd") + "_" + DateTime.Now.ToString("hhmmss") + ".csv";

            //RVUAT_MUFG_Circle_20210329_090103
